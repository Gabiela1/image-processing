...
import { DynamicLines, ColorExplosions, ProgressBar, DynamicBarChart, ColorCurtainEffect, StretchEffect} from "./proyectofinal.js";
...
let colorCurtainEffect: ColorCurtainEffect;
...
let stretchEffect: StretchEffect;

function initStretchEffect() {
  stretchEffect = new StretchEffect(ctx, pantalla2.canvas.width, pantalla2.canvas.height, 1, 0.01);
}

function animateStretchEffect() {
  const img = imgLocal.getImage();
  stretchEffect.update();
  stretchEffect.draw(img);

  requestAnimationFrame(animateStretchEffect);
}

function startStretchEffect() {
  initStretchEffect();
  animateStretchEffect();
}
...
document.getElementById("startStretchEffect").addEventListener('click', startStretchEffect, false);